...
export async function generate(options: GeneratorOptions) {
  ...
  const configLocator = (rawInline['config'] ||
    ...
    rawInline['configFile']) as string | undefined;
  ...
  if (configLocator) {
    ...
    try {
      ...
      console.error(
        '[prisma-trpc-generator] Failed to read config file at',
        configLocator,
        err,
      );
  ...
  if (config.auth !== false) {
    ...
    if (strategy === 'session') {
      imports.push(
        `import { getUser as _getUser } from '${rel(sessionCfg.getUserPath)}';`,
      );
      body.push(`const _resolveUser = async (req: any) => _getUser(req);`);
    } else if (strategy === 'jwt') {
      imports.push(
        `import { verifyToken as _verifyToken, getUserFromPayload as _getUserFromPayload } from '${rel(jwtCfg.verifyPath || jwtCfg.getUserFromPayloadPath)}';`,
      );
      ...
      body.push(
        `  const token = raw?.startsWith(scheme) ? raw.slice(scheme.length).trim() : raw;`,
      );
      body.push(
        `  const secret = process.env['${jwtCfg.secretEnv ?? 'JWT_SECRET'}'] || '';`,
      );
      ...
      imports.push(
        `import { resolveUser as _resolve } from '${rel(customCfg.resolverPath)}';`,
      );
    ...
    const authHelpersPath = path.resolve(
      outputDir,
      'routers',
      'helpers',
      'auth.ts',
...
`);
    ...
    if (config.withZod) {
      ...
      servicesSchemasImportBase = (
        schemasBaseFromServices.startsWith('.')
    ...
    for (const model of models) {
      ...
      const methodLine = (
        ...
        const schemaInputType = config.withZod
          ...
          : null;
        const parsed =
          config.withZod && schemaInputType
...
export function makeServices(ctx: Context) {
  return {
    ${models
      ...
      .map(
        (m) =>
          `${m.name[0].toLowerCase() + m.name.slice(1)}: new ${m.name}Service(ctx)`,
      ) // e.g., user: new UserService(ctx)
      .join(',\n    ')}
  } as const;
}
`);
    ...
    if (enabled) {
      ...
      for (const modelOperation of modelOperations) {
        ...
        for (const opType of modelActions) {
          ...
          const isFindFirst = ['findFirst', 'findFirstOrThrow'].includes(
            normalized,
          );
          const isFindManyLike = ['findMany', 'aggregate', 'count'].includes(
            normalized,
          );
          const isGroupBy = normalized === 'groupBy';
          const isCreate =
            normalized === 'create' ||
            normalized === 'createMany' ||
            normalized === 'createManyAndReturn';
          const isUpdate =
            normalized === 'update' ||
            normalized === 'updateMany' ||
            normalized === 'updateManyAndReturn' ||
            normalized === 'upsert';
          ...
          if (isUnique) {
            ...
            if (normalized === 'upsert')
              input = {
                where: toWhereUnique(model),
                update: buildUpdateData(model),
                create: buildCreateData(model),
              };
          } else if (isFindFirst) {
...
`;
      await fs.writeFile(
        path.resolve(adaptersDir, 'openapi.ts'),
        adapterTs,
        'utf8',
      );
    }
  ...
  if (postmanEnabled) {
    ...
    const collection = {
      ...
      item: items,
      variable: [{ key: envName, value: endpoint, type: 'string' }],
    } as const;
