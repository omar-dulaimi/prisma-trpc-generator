...
async function main() {
  ...
  const ctx = createContext({ req: { headers: {} } as any, res: {} as any, info: undefined } as any);
...
const router = t.router({
  ...
  boom: t.procedure
    .use(requestIdMiddleware)
    .query(({ ctx }) => {
      throw new trpc.TRPCError({ code: 'BAD_REQUEST', message: `boom r:${ctx.requestId ?? 'none'}` });
    }),
});
